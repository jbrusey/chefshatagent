...
def _sample_seat_agents(
    ...
    if seat_count < 2:
        raise RuntimeError(f"Expected at least 2 seats for Elo evaluation, got {seat_count}")

    opponents_needed = seat_count - 1
    if len(evaluation_pool) >= opponents_needed:
        chosen = list(rng.choice(evaluation_pool, size=opponents_needed, replace=False))
    else:
        chosen = list(rng.choice(evaluation_pool, size=opponents_needed, replace=True))

    return [snapshot_path, *chosen]
...
def evaluate_snapshot_elo(
    ...
    try:
        seat_count = adapter.num_seats(base_env)
        if hasattr(base_env, "startExperiment"):
            with _silence():
                base_env.startExperiment(
                    playerNames=[f"Elo{i}" for i in range(seat_count)],
                    logDirectory="log",
                    verbose=False,
                )
        for _ in range(games):
            seat_agents = _sample_seat_agents(
            ...
            if len(match_scores) != len(seat_agents):
                raise RuntimeError(
                    f"Expected {len(seat_agents)} terminal scores, got {len(match_scores)}"
                )
